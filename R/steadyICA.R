...
grad.JdCov <- function(S){
  ...
  G.grad <- list()
  for (i in 1:d) {
    G.grad[[i]] <- matrix(1, n, n)
    for (j in 1:d) {
      if (j == i){
        G.grad[[i]] <- G.grad[[i]]*der.U[[j]]
      }else{
        G.grad[[i]] <- G.grad[[i]]*U_mat[[j]]
      }
    }
    m.grad[, i] <- 2*rowSums(G.grad[[i]]) / (n^2)
  }
  m.grad
}

# function for computing U matrix; here we set c = 1.
U_mat <- function(A){
  list.U <- list()
  # compute U matrix
  for (i in 1:ncol(A)) {
    dx <- as.matrix(dist(A[, i]))
    list.U[[i]] <- -energy::D_center(dx) + 1
  }
  list.U
}

# function for computing derivative
der.U <- function(A){
  n <- nrow(A)
  der.U <- list()
  for (i in 1:ncol(A)){
    X.i <- A[,i]
    X.i.mat <- matrix(X.i, nrow = n, ncol = n)
    diff.mat <- t(X.i.mat) - X.i.mat # this matrix is used to construct sign(X_k-X_v)
    sign.mat <- -1L*(diff.mat < 0) + 1L*(diff.mat > 0)
    colsum.sign.mat <- matrix(colSums(sign.mat)/n, nrow = n, ncol = n)
    der.U[[i]] <- colsum.sign.mat - t(sign.mat) # set k be row, l be column
  }
  der.U
}


JdCovICA <- function(X, n.comp = ncol(X), w.init= NULL, PIT = FALSE, bw='SJ',adjust=1,whiten = FALSE, irlba = FALSE, symmetric=FALSE, eps = 1e-08, alpha.eps = 1e-08, maxit = 100, method = c('Cpp','R'), verbose = FALSE) {
  p <- ncol(X)
  d <- n.comp
  pd <- p * n.comp
  oldW=w.init
  ...
  if(symmetric) {
    deltaS <- grad.JdCov(S)*fS
  } else {
    deltaS <- grad.JdCov(S)*fS
  }
  # compute the current JdCov value
  S_list <- lapply(seq_len(ncol(S)), function(i) as.matrix(S[,i]))
  curF <- jdcov::jdcov.stat(S_list, cc=1, stat.type = "V")
  # curF <- multidcov(S,symmetric=symmetric)
  deltaW <- crossprod(Z,deltaS)
  Table <- NULL
  iter <- 1
  ...
  while (iter < maxit) {
    ...
    # compute new function value of JdCov
    S_list <- lapply(seq_len(ncol(S)), function(i) as.matrix(S[,i]))
    newF <- jdcov::jdcov.stat(S_list, cc=1, stat.type = "V")
    # newF <- multidcov(S=S,symmetric=symmetric)
    while (newF >= curF) { #Search for alpha that reduces f
      alpha <- alpha/2
      if(alpha < alpha.eps) {
    ...
    if(verbose) message('iter: ',rowTable[1],'; newF: ',round(rowTable[2],6),'; alpha: ',alpha)
    Table <- rbind(Table, rowTable)
    if(symmetric) deltaS <- grad.JdCov(S)*fS else deltaS <- grad.JdCov(S)*fS
    deltaW <- crossprod(Z,deltaS)
    iter <- iter+1
    if (deltaF < eps) break
  }
  ...
  if(symmetric) colnames(Table)=c('Iter','JdCov','alpha') else colnames(Table)=c('Iter','JdCov','alpha')
